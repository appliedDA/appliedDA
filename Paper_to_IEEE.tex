...
\begin{itemize}
    \item \textbf{T5-Small:} An encoder-decoder (Seq2Seq) model with approximately 60 million parameters. T5 was chosen for its unified ``text-to-text'' framework, which is optimized for conditional generation tasks where the output must conform to a rigid structural template.
    \item \textbf{GPT-2 (Base):} A decoder-only autoregressive model with 124 million parameters. This serves as a baseline to evaluate how unidirectional context handles the causal relationship between fault descriptions and resolutions.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Learning Rate:} $5 \times 10^{-5}$ with a linear warmup.
    \item \textbf{Sequence Length:} Max input and output length set to 512 tokens, utilizing truncation for long technical logs.
    \item \textbf{Hardware:} Training was executed on a single NVIDIA GPU (16 GB VRAM) with 30 GB of system RAM.
    \item \textbf{Regularization:} Early stopping was triggered if the validation loss failed to improve for three consecutive epochs to prevent overfitting to customer-specific patterns.
\end{itemize}
